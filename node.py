...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for possible_split in possible_splits:
            left_child = y[:possible_split]
            right_child = y[possible_split:]

            left_pos = (left_child == 1).sum()
            left_neg = (left_child == 0).sum()
            right_pos = (right_child == 1).sum()
            right_neg = (right_child == 0).sum()
            ...
            gini_right = 1 - (right_pos/right)**2 - (right_pos/right)**2
